...
interface Equations {
  ...
  equation: string;
  solution: string;
  steps: EquationStep[] | string[] | unknown; // Support multiple possible types
}
...
const isStringArray = (steps: any): steps is string[] => {
  return Array.isArray(steps) && steps.length > 0 && typeof steps[0] === 'string';
};

// Type guard for equation step arrays
const isEquationStepArray = (steps: any): steps is EquationStep[] => {
  return Array.isArray(steps) && steps.length > 0 && isStepObject(steps[0]);
};
...
const Equations = ({ params }: { params: { equations: string } }) => {
  ...
  // Colors to match app color scheme
  const colors = {
    background: '#fbf7ef',
    primary: '#f8b84e',
    text: '#2c2e33',
  ...
  const fetchEquationsDetails = useCallback(async () => {
    ...
    // Fetch only equations that belong to this specific study guide
    ...
    if (data) {
      setStudyGuideTitle(data.title);
      
      // Fetch all equations with the same title
      const { data: allEquations, error: eqError } = await supabase
        .from("study_guide")
        .select("id, title, equation, solution, steps")
        .eq("title", data.title);
      ...
      } else if (allEquations) {
        // Process the data to handle different step formats
        const processedData: Equations[] = allEquations.map((item: {
          id: number;
          title: string;
          equation: string;
          solution: string;
          steps: string | EquationStep[] | unknown;
        }): Equations => {
          return {
            ...item,
          ...
          >
            <Box>
              <Breadcrumbs aria-label="breadcrumb">
                ...
                <Typography color={colors.text}>{studyGuideTitle}</Typography>
              </Breadcrumbs>
              <Typography variant="h4" sx={{ mt: 1, color: colors.text }}>
                {studyGuideTitle} Study Guide
              </Typography>
            </Box>
            <Button
              variant="contained"
